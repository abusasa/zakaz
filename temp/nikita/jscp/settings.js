...
let settings = {
    ...
    sequence: 'МЕН СЕНІ ЖАҚСЫ КӨРЕМІН|Я ТЕБЯ ЛЮБЛЮ|I LOVE YOU| ЗУМРАТ!❤',
...
function resetWebsiteState() {
    ...
    if (typeof S !== 'undefined' && S.UI && window.settings) {
        S.UI.reset(true);
        const sequence = `|#countdown ${window.settings.countdown}|${window.settings.sequence}|#popup|#gift|`;
...
function initializeDefaultSettings() {
    ...
    window.settings = {
        ...
        matrixColor1: '#ff69b4', // Màu pink mẫu
        matrixColor2: '#ff1493', // Màu pink mẫu
        sequence: 'С ДНЁМ РОЖДЕНИЯ!|СОЛНЫШКО💓|Я ТЕБЯ ЛЮБЛЮ БЕЗУМНО|НИКИТАА!❤',
        sequenceColor: '#ff69b4', // Màu pink mẫu
        gift: './image/gif.gif',
        enableBook: true,
        enableHeart: true,
        ...
        pages: [
            ...
            { 
                image: './image/photos/b2.png', // Menggantikan fjr3qh.png (Index 2)
                content: 'Никита, ты — самое прекрасное, что произошло со мной в этом году и вообще в моей жизни. С тобой каждый час, каждая минута и каждая секунда становятся особенными. Я очень ценю всё время, которое мы проводим вместе, и хочу, чтобы оно продолжалось как можно дольше — до конца моей жизни, до моего последнего вздоха!💗' 
            },
            { 
                image: './image/photos/b3.png', // Menggantikan 8u5tf7.png (Index 3)
            },
            ...
            { 
                ...
                content: 'Ты самый особенный человек в моей жизни, самый значимый и любимый. В тебе прекрасно абсолютно всё: твои глаза, волосы, запах, улыбка, руки… ты прекрасен. Благодаря тебе я наконец почувствовала, насколько «любовь» может быть искренней, чистой, тёплой и прекрасной.' 
            }
        ]
    };
...
function applyLoadedSettings() {
    ...
    // Reset and restart the sequence với màu mới и popup
    S.UI.reset(true);
    const sequence = `|#countdown ${settings.countdown}||${settings.sequence}|#popup|#gift|`;
    S.UI.simulate(sequence);
