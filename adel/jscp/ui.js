...
S.ShapeBuilder = (function () {
    ...
    function wrapText(text, maxWidth) {
        ...
        if (words.length <= 1) return [safeText];
        ...
        for (let i = 1; i < words.length; i++) {
            ...
            if (shapeContext.measureText(candidate).width <= maxWidth) {
    ...
    return {
        ...
        letter: function (l) {
            ...
            const baseFontSize = (isMobile || isSmallScreen) ? 220 : 360;
            const safeText = String(l ?? '').trim();
            ...
            const maxWidth = shapeCanvas.width * 0.82;
            const maxHeight = shapeCanvas.height * 0.72;
            const lines = wrapText(safeText, maxWidth);
            const lineHeight = baseFontSize * 1.1;
            const totalHeight = lines.length * lineHeight;
            const fontScale = totalHeight > maxHeight
                ? Math.max(90, (maxHeight / totalHeight) * baseFontSize)
                : baseFontSize;

            setFontSize(fontScale);
            shapeContext.clearRect(0, 0, shapeCanvas.width, shapeCanvas.height);

            const startY = (shapeCanvas.height - (lines.length * fontScale * 1.1)) / 2 + fontScale * 0.35;
            lines.forEach((line, index) => {
                shapeContext.fillText(line, shapeCanvas.width / 2, startY + index * fontScale * 1.1);
            });
